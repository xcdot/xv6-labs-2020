...
userinit(void)
{
  ...
  p->sz = PGSIZE;
  // 同步程序内存映射到进程内核页表
  kvmcopymappings(p->pagetable, p->kernel_pagetable_indep, 0, p->sz);
...
growproc(int n)
{
  ...
  if(n > 0){
    ...
    //Lab3.3：内核页表中的映射同步扩大
    if (kvmcopymappings(p->pagetable, p->kernel_pagetable_indep, sz, n) != 0) {
    ...
    sz = kvmdealloc(p->kernel_pagetable_indep, sz, sz+n);
  }
  p->sz = sz;
  return 0;
}
...
fork(void)
{
  ...
  if(uvmcopy(p->pagetable, np->pagetable, p->sz) < 0 ||
     kvmcopymappings(np->pagetable, np->kernel_pagetable_indep, 0, p->sz) < 0){
    freeproc(np);
    release(&np->lock);
    return -1;
  }
